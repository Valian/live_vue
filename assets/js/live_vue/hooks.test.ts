import type { Hook, LiveVueApp } from './types'
...
function createMockLiveViewHook(elementAttributes: Record<string, string> = {}) {
  const mockElement = {
...
describe('getVueHook', () => {
    ...
    it('should parse props from data-props attribute', async () => {
      ...
      mockHookContext.el.getAttribute.mockImplementation((name: string) => {
        if (name === 'data-name')
          return 'TestComponent'
        if (name === 'data-props')
          return JSON.stringify(mockProps)
        return null
      })
    ...
    it('should parse handlers from data-handlers attribute', async () => {
      ...
      mockHookContext.el.getAttribute.mockImplementation((name: string) => {
        ...
        if (name === 'data-handlers')
          return JSON.stringify(mockHandlers)
        return null
      })
    ...
    it('should parse slots from data-slots attribute', async () => {
      ...
      mockHookContext.el.getAttribute.mockImplementation((name: string) => {
        if (name === 'data-name')
          return 'TestComponent'
        if (name === 'data-slots')
          return JSON.stringify(mockSlots)
        return null
      })
  ...
  describe('updated lifecycle', () => {
    beforeEach(async () => {
      ...
      mockHookContext.el.getAttribute.mockImplementation((name: string) => {
        if (name === 'data-name')
          return 'TestComponent'
        if (name === 'data-props')
          return JSON.stringify({ message: 'initial', count: 1 })
        return null
      })
    ...
    it('should apply props diff when data-use-diff is true', async () => {
      ...
      mockHookContext.el.getAttribute.mockImplementation((name: string) => {
        if (name === 'data-name')
          return 'TestComponent'
        if (name === 'data-props')
          return JSON.stringify(complexInitialProps)
        return null
      })
      await vueHook.mounted!.call(mockHookContext)
      ...
      mockHookContext.el.getAttribute.mockImplementation((name: string) => {
        if (name === 'data-use-diff')
          return 'true'
        if (name === 'data-props-diff')
          return JSON.stringify(mockPropsDiff)
        return null
      })
    ...
    it('should update slots', () => {
      ...
      mockHookContext.el.getAttribute.mockImplementation((name: string) => {
        if (name === 'data-slots')
          return JSON.stringify(newSlots)
        return null
      })
    ...
    it('should handle handlers in updated props', () => {
      ...
      mockHookContext.el.getAttribute.mockImplementation((name: string) => {
        ...
        if (name === 'data-handlers')
          return JSON.stringify(newHandlers)
        return null
      })
    ...
    it('should maintain reactive props during updates', async () => {
      ...
      mockHookContext.el.getAttribute.mockImplementation((name: string) => {
        if (name === 'data-use-diff')
          return 'false'
        if (name === 'data-props')
          return JSON.stringify({ message: 'updated', count: 2 })
        return null
      })
