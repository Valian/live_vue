defmodule LiveVue do
  ...
  rewrites `<.vue>` tags to inject shared props and `v-socket` automatically. You usually
  don't need to pass `v-socket` manually unless you call `LiveVue.vue/1` directly or bypass
  that `~H` override.
  ...
  ### Optional Attributes
    ...
    * `v-socket` (LiveView.Socket) - LiveView socket. Usually injected automatically for `<.vue>`
      tags in standard `~H` templates; pass it manually when calling `LiveVue.vue/1` directly or
      bypassing `LiveVue.SharedPropsView`
