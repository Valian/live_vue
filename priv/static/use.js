 ...
 */
export const useLiveUpload = (uploadConfig, options) => {
    const live = useLiveVue();
    ...
    onMounted(() => {
        if (!inputEl.value) {
            ...
            const updateEntryRefs = () => {
                ...
                const joinEntries = (entries) => entries.map(e => e.ref).join(",");
            ...
            input.__unwatchConfig = unwatchConfig;
    ...
    const progress = computed(() => {
        ...
        if (allEntries.length === 0)
            return 0;
        const totalProgress = allEntries.reduce((sum, entry) => sum + (entry.progress || 0), 0);
    ...
    const addFiles = (input) => {
        ...
        if (input instanceof DataTransfer) {
            ...
            input.forEach(f => dataTransfer.items.add(f));
 ...
 */
export const useEventReply = (eventName, options) => {
    const live = useLiveVue();
...
export const useLiveConnection = () => {
    ...
    onUnmounted(() => {
        ...
        const refs = [openRef, closeRef, errorRef].filter(ref => ref !== null);
        if (refs.length > 0) {
            socket.off(refs);
        }
    });
